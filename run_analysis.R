...
if (!file.exists(ds_dir)) {
  ...
  +     dest_file <- "C:/Users/szhang/Desktop/class materials/Dataset.zip"
  +     download.file(source_url, destfile = dest_file)
  +     unzip(dest_file)
  +     if (!file.exists(ds_dir)) 
...
##Uses list of activity values to describe test/ tranining labels
...
## Take a data set caturing results  of feature tests and associate columns with individual features
## Params: ds ... activity dataset
# Returns: the original dataset with columns indicating which feature it describes
describe_ft_ds <- function(ds){
  col_names <- gsub("-", "_", features$V2)
  col_names <- gsub("[^a-zA-Z\\d_]", "", col_names)
  names(ds)<- make.names(names= col_names, unique= TRUE, allow= TRUE)
  ds
}

## column name for test participants
...
merge_data <- rbind(
              cbind(train_ft, train_lbl, train_sub),
              cbind(test_ft, test_lbl, test_sub)
...
write.table(tidy, file = "tidy_data.csv", sep = ",", row.names = FALSE)